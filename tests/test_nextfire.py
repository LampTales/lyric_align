...
from lyric_align.ctc_text import build_target, group_ctc_tokens
...
def test_nextfire_target_expands_mora_and_preserves_long_vowels_and_geminates():
    records, coverage, text = build_target(dict(reading="きゃっとコーヒーヴァイオリン"), VOCAB)
...
def test_mixed_english_uses_surface_and_particle_uses_pronunciation():
    ...
    records, coverage, text = build_target(line, VOCAB)
    ...
    line.update(start_ms=0, end_ms=len(tokens) * 30, mora=group_ctc_tokens(tokens))
...
def test_untransliterated_script_counts_against_coverage():
    _, coverage, text = build_target(dict(reading="あ漢字"), VOCAB)
    assert text == "a漢字"
    assert coverage == pytest.approx(1 / 3)
    assert build_target(dict(reading="！？"), VOCAB) == ([], 0, "")
...
def test_real_sudachi_path_preserves_mora_and_long_vowels():
    ...
    records, coverage, target = build_target(line, VOCAB)
...
def test_english_punctuation_does_not_invent_kigou_pronunciation():
    ...
    line.update(start_ms=0, end_ms=2000, mora=group_ctc_tokens(tokens), ctc_window={'profile': 'nextfire'})
    units = _build_display_units(line)
